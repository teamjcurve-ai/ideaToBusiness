...
export default function Step7Page({ params }: PageProps) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      await api.put(`/projects/${params.id}/steps/7`, { status: 'IN_PROGRESS' });
      ...
      await api.put(`/projects/${params.id}/steps/7`, {
        content: generatedContent,
        status: 'COMPLETED',
      });

      setContent(generatedContent);
      updateStep(params.id, 7, { content: generatedContent, status: 'COMPLETED' });
    } catch (error: any) {
        ...
        {!content ? (
          ...
          <>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
              ...
              <button
                onClick={() => router.push(`/projects/${params.id}/step/8`)}
